...
# Define levels
mean_levels = np.arange(-2, 36, 2)  # SST typically ranges from -2°C (polar) to ~35°C (tropics)
...
contour1 = plot_map(axes[0, 0], obs_annual_data, "Observation SST Annual Mean", mean_levels, mean_cmap)
...
contour3 = plot_map(axes[1, 0], model1_annual_data, "CMIP7 SST Annual Mean", mean_levels, mean_cmap)
...
if model2_annual_data is not None:
    contour4 = plot_map(axes[2, 0], model2_annual_data, "CMIP6 SST Annual Mean", mean_levels, mean_cmap)
...
# Save the plot
output_file = os.path.join(output_dir, f"{var}_annual_comparison_sst_{projection}.png")
