...
#  Date: March 2025
#  Version: 1.2 (Seasonal SST Bias Plot)
#
# ==============================================================================
...
var = sys.argv[4]  # SST variable name in models
obs_var = sys.argv[5]  # SST variable name in observations
...
try:
    ...
    # Extract SST variable and select first time step
...
# Define levels
mean_levels = np.arange(-2, 36, 2)  # SST range from -2°C (polar) to ~35°C (tropics)
bias_levels = np.arange(-5, 5.5, 0.5)  # Bias range
...
contour1 = plot_map(axes[0, 0], obs_season_data, f"Observation SST {season} Mean", mean_levels, mean_cmap)
...
contour3 = plot_map(axes[1, 0], model1_season_data, f"CMIP7 SST {season} Mean", mean_levels, mean_cmap)
...
if model2_season_data is not None:
    contour4 = plot_map(axes[2, 0], model2_season_data, f"CMIP6 SST {season} Mean", mean_levels, mean_cmap)
...
# Save the plot
output_file = os.path.join(output_dir, f"{var}_seasonal_comparison_sst_{season}_{projection}.png")
